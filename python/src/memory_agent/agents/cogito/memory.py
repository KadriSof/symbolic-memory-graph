...
from .schemas import ReasoningDecision
...
class WorkingMemory:
    """
    Per-turn cognitive scratchpad.

    WorkingMemory contains temporary information required to process
    the current user request. It is not long-term memory.
    """

    # Execution / routing
    current_step: int = 0
    next_action: ReasoningDecision | None = None
    ...
    clarification_requests_limit: int = 3
    ...
    def clarification_cycle_exhausted(self) -> bool:
        return self.clarification_requests_count >= self.clarification_requests_limit

    def should_request_clarification(self) -> bool:
        if self.clarification_cycle_exhausted():
            return False

        return (
            self.next_action == ReasoningDecision.REQUEST
            or self.has_conflicts()
            or self.has_gaps()
        )

    def register_clarification_request(self) -> None:
        """Record that a clarification request was issued."""
        if self.clarification_cycle_exhausted():
            raise RuntimeError("Clarification cycle limit has already been reached.")

        self.clarification_requests_count += 1

    def is_ready_for_reply(self) -> bool:
        """Return True when reasoning produced a usable final answer."""
        return self.next_action == ReasoningDecision.REPLY and bool(
            self.final_answer.strip()
        )

    def to_prompt_context(self, max_chars: int = 4000) -> str:
        """Build concise cognitive context for the LLM."""
        ...
        if self.retrieved_knowledge:
            retrieved = str(self.retrieved_knowledge)[:1000]

            context.append(f"## Retrieved Knowledge\n{retrieved}")

        if self.conflicts:
            context.append(f"## Conflicts\n{str(self.conflicts)[:1000]}")
        ...
        if self.clarification_response:
            context.append(
                f"## User Clarification\n" f"{self.clarification_response[:1000]}"
            )

        if self.reasoning:
            context.append(f"## Previous Reasoning\n" f"{self.reasoning[:1500]}")
    ...
    def from_dict(
        ...
        if next_action is not None:
            payload["next_action"] = ReasoningDecision(next_action)
        ...
        for entity in payload.get(
            ...
            entities.append(
                Entity(**{key: value for key, value in entity.items() if key != "id"})
            )
        ...
        payload["extracted_relations"] = relations
